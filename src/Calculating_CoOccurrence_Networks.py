...
if randomized_control:    
    if preserve_years:
        file_name = 'synthetic_control_cooccurrence_%s%s_preserve_years_%s'%(n_years_label, entity_column, class_system)
    else:
        file_name = 'synthetic_control_cooccurrence_%s%s_no_preserve_years_%s'%(n_years_label, entity_column, class_system)
    if chain:
        file_name += '_chain.h5'
    else:
...
#     df.ix['Class_CoOccurrence_Count_%s'%entity_column] = networks
    df = networks
    store.put('/empirical_cooccurrence_%s%s_%s'%(n_years_label, entity_column, class_system), df, 'table', append=False)
    store.close()
